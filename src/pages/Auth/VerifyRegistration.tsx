...
export default function VerifyRegistration() {
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      setRemainingAttempts(null); // Reset attempts tracking for verification input
        ...
        <Card hoverLift={false} className="p-8">
          ...
          {remainingAttempts !== null && remainingAttempts > 0 && remainingAttempts <= 3 && lockoutTime <= 0 && (
            <div className="mb-6 p-4 rounded-xl bg-amber-50 text-amber-700 text-xs font-bold border border-amber-200/50 flex items-center gap-2 animate-pulse">
              ...
              <span>Tersisa {remainingAttempts} kali percobaan verifikasi/resend sebelum terblokir.</span>
            </div>
          )}
          ...
          <div className="mt-4 text-center text-xs">
            ...
            >
              {resendCountdown > 0 ? `Kirim Ulang (${resendCountdown}s)` : lockoutTime > 0 ? 'Kirim Ulang OTP (Terkunci)' : 'Kirim Ulang OTP'}
            </button>
          </div>
